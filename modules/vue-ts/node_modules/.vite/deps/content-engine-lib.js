...
var k = class {
  ...
  isRunning;
  constructor() {
    this.startTime = 0, this.endTime = 0, this.isRunning = false;
  }
  // Starts the timer
  start() {
    if (this.isRunning)
      throw new Error("Benchmark has already started.");
    this.startTime = performance.now(), this.isRunning = true;
  }
  // Stops the timer and records the end time
  stop() {
    if (!this.isRunning)
      throw new Error("Benchmark hasn't started.");
    this.endTime = performance.now(), this.isRunning = false;
  }
  ...
  lib: {
    ...
    set: async (t) => {
      ...
      for (const [r, n] of e.entries()) {
        ...
        if (l && (l = l.replace("{*}", `${s.name}`)), console.log(`_lazy_src: ${l}`), console.log(`_src: ${o}`), c.lib.l.hasOwnProperty(`${i}`) == false) {
          if (/^[a-zA-Z0-9]/.test(o) && o.includes("/") == false && c.lib.inbuilt_lib.indexOf(`${s.name}`) === -1)
            if (l)
              o = l;
            else
    ...
    get: async (t) => {
      ...
      if (c.lib.l.hasOwnProperty(`${r}`) == false) {
        let n = [{
          renderer_src: t.name,
          hydrator_src: t.name,
          editor_src: t.name,
